...
```{r}
ggplot(data=sdr_data,aes(x=goal_4_score, y=goal_6_score, color = goal_2_score))+geom_point()
```
    COuntries with a high goal 6 score also have a high goal 4 score. Majority of the countries seem to have a high goal 2 score while a few have a low goal 2 score 
...
ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = goal_5_score)) +
  geom_point()
```
Countries with A high goal 4 score also have a high goal 5 score
...
```{r}
ggplot(sdr_data, Aes(x=goal_3_score, Y -= country))+geom_bar(stat="identify")
...
MENA_sdr_data <- sdr_data %>% 
  filter(regions_used_for_the_sdr == "MENA") # filtering for MENA countries only
```


```{r}
OECD_sdr_data <- sdr_data %>% 
  filter(regions_used_for_the_sdr == "OECD") # filtering for OECD countries only
```
...
```
```{r}
ggplot(OECD_sdr_data, aes(x= goal_14_score, y = country)) +
...
```{r}
ggplot(sdr_data, aes(x = goal_14_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
```{r}
ggplot(sdr_data, aes(x = goal_17_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interpretation: The Only Region With A Consistent Pattern Is The SUb-Saharan Africa more in the lower end the rest of the Regions dont have a consistent pattern and are found all over the chart.- DATAMakalii
...
```{r}
ggplot(OECD_sdr_data, aes(x= goal_17_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33,color="black")
```
Everyone In This chart is doing alright```
```{r}
OECD_sdr_data <- sdr_data %>%
filter(regions_used_for_the_sdr == "OECD"))
