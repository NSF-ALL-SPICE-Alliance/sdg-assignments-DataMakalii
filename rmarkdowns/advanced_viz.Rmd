...
```
```{r}
advanced_Viz_Data <- read_csv(here("data/SDR-2023-Data.csv"))
```
...
```{r, message=FALSE, warning=FALSE}
ggplot(sdr_data, aes(x = goal_4_score, y= goal_2_score, color=regions_used_for_the_sdr)) +
  geom_histogram() +
  theme_minimal() +
...
```{r}
goal_4_point <- ggplot(sdr_data, aes(x = goal_4_score, y= goal_2_score,  color=regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal () +
  scale_fill_viridis_d(option="magma") +
  labs(title = "Goal Differences ",
  x = "Score 4 goal !",
  y = "Goal 2 Scores",
  Color = "region")
  
  ggplotly(goal_4_point)
```
...
```{r}
goal_3_4_scatter_plot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr, Label=country)) +
  geom_point()+
  theme_minimal()+
  scale_fill_viridis_d(option="inferno")+
  labs(title= "Compare/Contrast Goals",
  x= "Goal 3 Score",
  y= "goal 4 Score",
  Color ="regions")
  
  ggplotly(goal_3_4_scatter_plot)
```
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("magma", goal_2_score)(goal_2_score), label = mytext)
```
```
...
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
    "Goal 12 Score: ", round(sdr_data_world_joined$goal_2_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("plasma", goal_12_score)(goal_12_score), label = mytext)
```
```
