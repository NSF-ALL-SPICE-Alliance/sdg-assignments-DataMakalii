...
```{r}
project_plot<- ggplot(sdr_data, aes(x = goal_6_score, 
                     y = goal_15_score,
                     label = country,
                     color= "magma")) +
  ...
  stat_cor() +
  theme_minimal()
ggplotly(project_plot)
```
        
  there is a positive correlation with these 2 SDG scores, it is a slight correlation, Most of the countries have SDG scores that are very similiar two each other. There is only a few with low scores in one area but high scores in the other. The Slope Rises in the begining and drops a tad bit, then rises back up on a gradual slope. Country Lativa,United Kingdom, Denmark, Finland, Sweeden ETC. is doing well in both SDG Scores.     