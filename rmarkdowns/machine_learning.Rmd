...
```{r}
k6 <- kmeans(sdr_data_imputed, centers = 6)
```

```{r}
fviz_cluster(k6, data = sdr_data_imputed) + theme_minimal()
```
...
```{r}
rf_slavery <- randomForest(normalized_score_sdg8_slavery ~ ., data = sdr_data_imputed, importance = TRUE)
```
...
```
```{r}
importance_ds <- as.data.frame(rf_slavery$importance)
```

```{r}
importance_ds_top_10 <- importance_ds %>%
  rownames_to_column(var = "variable") %>%
  slice_max(n = 10, order_by = `%IncMSE`)
```
...
```{r}
ggplot(importance_ds_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Slavery",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
```{r}
pdp::partial(rf_slavery, pred.var = "normalized_score_sdg3_pollmort", plot = TRUE)
```
  
```{r}
pd <- pdp::partial(rf_slavery, pred.var = c("normalized_score_sdg3_pollmort", "normalized_score_sdg6_water"))
# Default PDP
plotPartial(pd)
```
  Water Has A Big Part To Play In Pollution  Mortality.
